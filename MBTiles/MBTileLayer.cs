...
public class MBTileLayer : MapTileLayer
{
    ...
    private async Task FilePropertyChanged( string? file )
    {
        ...
        if( file != null )
        {
            ...
            if( tileData.Metadata.TryGetValue( "minzoom", out sourceName )
            && int.TryParse( sourceName, out int minZoom ) )
                MinZoomLevel = minZoom;

            if( tileData.Metadata.TryGetValue( "maxzoom", out sourceName )
            && int.TryParse( sourceName, out int maxZoom ) )
