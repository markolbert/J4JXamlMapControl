...
public class WmtsCapabilities
{
    ...
    {
        if( !capabilitiesUri.IsAbsoluteUri
        || ( capabilitiesUri.Scheme != "http" && capabilitiesUri.Scheme != "https" ) )
        {
            return ReadCapabilities( XDocument.Load( capabilitiesUri.ToString() ).Root,
                                     layerIdentifier,
                                     null );
        }

        await using var stream = await ImageLoader.HttpClient.GetStreamAsync( capabilitiesUri );

        return ReadCapabilities( XDocument.Load( stream ).Root,
                                 layerIdentifier,
                                 capabilitiesUri.ToString() );
    }

    public static WmtsCapabilities? ReadCapabilities(
        XElement? capabilitiesElement,
    ...
    )
    {
        ...
        if( !string.IsNullOrEmpty( layerIdentifier ) )
        {
            layerElement = contentsElement.Descendants( ns + "Layer" )
                                          .FirstOrDefault(
                                               e => e.Element( ows + "Identifier" )?.Value == layerIdentifier );
        ...
        var styleElement = layerElement.Descendants( ns + "Style" )
         ...
         ?? layerElement.Descendants( ns + "Style" ).FirstOrDefault();
        ...
        foreach( var tileMatrixSetId in tileMatrixSetIds )
        {
            var tileMatrixSetElement = capabilitiesElement.Descendants( ns + "TileMatrixSet" )
                                                          .FirstOrDefault(
                                                               e => e.Element( ows + "Identifier" )?.Value
                                                                == tileMatrixSetId );

            if( tileMatrixSetElement == null )
                throw new ArgumentException(
                    $"Linked TileMatrixSet element not found in Layer \"{layerIdentifier}\"." );
    ...
    public static string ReadUrlTemplate(
        XElement layerElement,
        string layerIdentifier,
        string style,
        string? capabilitiesUrl
    )
    {
        ...
        var resourceUrls = layerElement.Descendants( ns + "ResourceURL" )
                                       .ToLookup( e => e.Attribute( "format" )?.Value ?? "",
                                                  e => e.Attribute( "template" )?.Value ?? "" );

        if( resourceUrls.Any() )
        {
            var urlTemplates = resourceUrls.Contains( formatPng )
                ? resourceUrls[ formatPng ]
                : resourceUrls.Contains( formatJpg )
                    ? resourceUrls[ formatJpg ]
                    : resourceUrls.First();
            ...
            {
                var requestIndex =
                    capabilitiesUrl.IndexOf( "Request=GetCapabilities", StringComparison.OrdinalIgnoreCase );
                ...
                if( requestIndex > 0 )
                {
                    ...
                    var format = formats.Contains( formatPng )
                        ? formatPng
                        : formats.Contains( formatJpg )
                            ? formatJpg
                            : formats[ 0 ];
    ...
    public static WmtsTileMatrix ReadTileMatrix( XElement tileMatrixElement, string supportedCrs )
    {
        ...
        if( string.IsNullOrEmpty( valueString )
        || ( topLeftCornerStrings = valueString.Split( new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries ) )
          .Length
         < 2
        || !double.TryParse( topLeftCornerStrings[ 0 ],
                             NumberStyles.Float,
                             CultureInfo.InvariantCulture,
                             out double left )
        || !double.TryParse( topLeftCornerStrings[ 1 ],
                             NumberStyles.Float,
                             CultureInfo.InvariantCulture,
                             out double top ) )
        {
    ...
    public string? LayerIdentifier { get; private init; }
    public WmtsTileSource? TileSource { get; private init; }
    public List<WmtsTileMatrixSet>? TileMatrixSets { get; private init; }
}