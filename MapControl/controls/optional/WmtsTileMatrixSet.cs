...
public class WmtsTileMatrixSet
{
    ...
    public WmtsTileMatrixSet(string identifier, string supportedCrs, IEnumerable<WmtsTileMatrix> tileMatrixes)
    {
        ...
        var tmList = tileMatrixes.ToList();

        if( !tmList.Any() )
            throw new ArgumentException( "The tileMatrixes argument must not be null or an empty collection.",
                                         nameof( tileMatrixes ) );

        Identifier = identifier;
        SupportedCrs = supportedCrs;
        TileMatrixes = tmList.OrderBy( m => m.Scale ).ToList();
    }
}